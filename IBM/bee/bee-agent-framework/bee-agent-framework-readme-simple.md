...
- **Battle-tested.** Bee Agent Framework is at the core of [BeeAI](https://iambee.ai), a powerful platform for building chat assistants and custom AI-powered apps. BeeAI is in a closed beta, but already used by hundreds of users. And it's [fully open-source](https://github.com/i-am-bee/bee-ui) too!
...
workflow.addAgent({
  name: "Researcher",
  instructions:
    "You are a researcher assistant. Respond only if you can provide a useful answer.",
...
workflow.addAgent({
  ...
  instructions:
    "You are a weather assistant. Respond only if you can provide a useful answer.",
...
you can run this example after local installation, using the command `yarn start examples/agents/simple.ts`
...
To run an arbitrary example, use the following command `yarn start examples/agents/bee.ts` (just pass the appropriate path to the desired example).
...
The Bee Agent Framework is an open-source project and we love contributions.
